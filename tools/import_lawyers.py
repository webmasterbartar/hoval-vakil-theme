...
برای ایمپورت روی سرور بدون REST از PC، بستهٔ manifest + images را با
``python tools/build_import_bundle.py --checkpoint ... --out ...`` بسازید
(راهنما داخل همان فایل).
"""
...
from hvl_checkpoint_payload import (
    external_id_for,
    jdatetime,
    lawyer_featured_image_filename,
...
# Server-side cap (see hovalvakil_lawyer_batch_max_items in PHP); client never exceeds this.
WP_LAWYER_BATCH_MAX = 50
MAX_BATCH_RETRIES = 8
MAX_ITEM_RETRIES = 5
...
def apply_config_env_defaults(args: argparse.Namespace, file_cfg: Dict[str, Any]) -> None:
    ...
    if file_cfg.get("batch_size") is not None and not _argv_has_flag("--batch-size"):
        try:
            args.batch_size = int(file_cfg["batch_size"])
        except (TypeError, ValueError):
            pass
    if file_cfg.get("media_sleep") is not None and not _argv_has_flag("--media-sleep"):
        try:
            args.media_sleep = float(file_cfg["media_sleep"])
        except (TypeError, ValueError):
            pass
    ...
    *,
    ...
    if image_mode == "rest_media" and not dry_run:
        ...
        if lp is not None:
            ...
            if media_sleep and media_sleep > 0:
                time.sleep(float(media_sleep))
